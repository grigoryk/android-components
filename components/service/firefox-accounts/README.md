...
`AccountMigration` provides facilities to securely query auth providers for available accounts. It may be used
directly in concert with a low-level `FirefoxAccount.migrateFromSessionTokenAsync`, or via the high-level `FxaAccountManager`:

```kotlin
val availableAccounts = accountManager.migratableAccounts(context)
// Display a list of accounts to the user, identified by account.email and account.sourcePackage
// Or, pick the first available account. They're sorted in an order of internal preference.
val selectedAccount = availableAccounts[0]
launch {
    val result = accountManager.migrateAccountAsync(selectedAccount).await()
    if (result) {
