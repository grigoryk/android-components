    ...
    return try {
        ...
        when (e) {
            is FxaUnauthorizedException -> {
                ...
                GlobalAccountManager.instance.encounteredAuthError()
 ...
 */
suspend fun <T> handleFxaExceptions(logger: Logger, operation: String, default: (error: FxaException) -> T, block: suspend () -> T): T {
 ...
 */
suspend fun handleFxaExceptions(logger: Logger, operation: String, block: suspend () -> Unit): Boolean {
